...
export default async function ProjectSlugPage({
  ...
  return (
    <main className="mx-auto w-full max-w-container flex-1 px-4 py-10">
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_17.5rem] lg:items-start">
        <div className="min-w-0 space-y-10">
          <header className="space-y-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {pills.map((pill) => (
                <li key={pill}>
                  <span className="inline-flex rounded-full border border-rule bg-fg/[0.03] px-3 py-1 text-xs font-medium text-foreground/90">
                    {pill}
                  </span>
                </li>
              ))}
          ...
          <div className="max-w-none border-t border-rule pt-8">
            {await MDXRemote({
          ...
          <nav
            ...
            className="flex flex-col gap-4 border-t border-rule pt-8 sm:flex-row sm:justify-between"
          >
            <div className="min-w-0 sm:max-w-[50%]">
              {prev ? (
                <Link
                  className="group block rounded-lg border border-rule p-4 transition hover:-translate-y-0.5 hover:shadow-md"
        ...
        <aside className="space-y-6 border-t border-rule pt-8 text-sm lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0">
          <div>
          ...
          <div>
            ...
            <ul className="mt-2 flex flex-wrap gap-2">
              {project.tags.map((tag) => (
                <li key={tag}>
                  <span className="inline-flex rounded-full border border-rule px-2 py-0.5 text-xs text-foreground/85">
                    {tag}
                  </span>
                </li>
              ))}
